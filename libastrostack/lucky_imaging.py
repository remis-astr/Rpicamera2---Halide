...
class QualityScorer:
    ...
    def score(self, image: np.ndarray) -> float:
        ...
        # IMPORTANT: float32 [0,1] → scale ×255 avant uint8 (astype(uint8) tronque → tout zéro!)
        def _to_uint8(img):
            if img.dtype == np.uint8:
                return img
            if img.max() <= 1.0 + 1e-6:  # plage [0,1]
...
class FrameAligner:
    ...
    def _create_planetary_aligner(self):
        """Crée un PlanetaryAligner configuré selon align_mode.
        align_mode: 1=Surface/phase FFT, 2=AP (pré-alignement DISK + corrections locales AP), 3=Hybrid"""
        if not _HAS_PLANETARY:
            print("[ALIGN] PlanetaryAligner non disponible, fallback surface/phase")
            return
        cfg = _PlanetaryConfig()
        # align_mode=2 (AP) utilise DISK pour le pré-alignement global rapide.
        # L'APStackEngine gère ensuite les corrections locales atmosphériques.
        # SURFACE mode retourne peak~0.03 avec seuil 0.3 → tous les alignements échouent.
    ...
    def set_reference(self, image: np.ndarray, explicit: bool = False):
        ...
        if self.config.align_mode in (2, 3):
            ...
            # align_mode=2 (AP) : stocker aussi self.reference pour que
            # _do_stack() puisse l'utiliser via self.aligner.reference is not None
            if self.config.align_mode == 2:
...
class ElitePoolStacker:
    ...
    def __init__(self, config: Optional['LuckyConfig'] = None):
        ...
        # AP Stacker (optionnel — activé si align_mode == 2)
        self.ap_engine = None   # type: Optional[Any]  # APStackEngine
    ...
    def _do_stack(self):
        ...
        try:
            # Récupérer frames + scores (nécessaire pour le mode AP)
            frames_and_scores = self._pool.get_frames_and_scores()  # [(score, frame), ...]

            if not frames_and_scores:
                return

            # Sigma-clipping des scores (optionnel)
            if self.config.elite_score_clip:
                scores_arr = np.array([s for s, _ in frames_and_scores], dtype=np.float64)
                mean_s, std_s = scores_arr.mean(), scores_arr.std()
                if std_s > 1e-9:
                    threshold = mean_s - self.config.elite_score_kappa * std_s
                    frames_and_scores = [(s, f) for s, f in frames_and_scores if s >= threshold]
                self._last_clipped_count = len(self._pool.get_frames_and_scores()) - len(frames_and_scores)
            else:
            ...
            scores = [s for s, _ in frames_and_scores]
            frames = [f for _, f in frames_and_scores]

            # ── Mode AP (Alignment Points) ──────────────────────────────────���─
            if self.ap_engine is not None and self.aligner.reference is not None:
                result = self.ap_engine.process(frames, scores, self.aligner.reference)
            else:
                # ── Modes classiques ─────────────────────────────────���────────
    ...
    def configure(self, **kwargs):
        ...
        if 'align_mode' in kwargs:
            ...
            self.config.align_enabled = (new_mode != 0)
            if new_mode == 2:
                # Activer AP engine si pas déjà présent
                if self.ap_engine is None:
                    try:
                        from .ap_stacker import APStackEngine
                        self.ap_engine = APStackEngine()
                        print("[ELITE POOL] AP Stacker activé")
                    except ImportError as e:
                        print(f"[ELITE POOL] APStackEngine non disponible: {e}")
            else:
                # Désactiver AP
                if self.ap_engine is not None:
                    self.ap_engine = None
                    print("[ELITE POOL] AP Stacker désactivé")
