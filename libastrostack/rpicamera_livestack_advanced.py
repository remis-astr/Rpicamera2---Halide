    ...
    def start(self):
        ...
        if self.use_lucky or self.config.lucky.enable:
            ...
            if self.config.lucky.buffer_mode == "elite":
                ...
                # Activer AP engine si align_mode == 2
                if lucky_config.align_mode == 2:
                    try:
                        from .ap_stacker import APStackEngine
                        self.lucky_stacker.ap_engine = APStackEngine()
                        print(f"[LUCKY] Mode AP (Alignment Points) activé")
                    except ImportError as e:
                        print(f"[LUCKY] APStackEngine non disponible: {e}")
                print(f"[LUCKY] Mode Pool Élite activé"
                      f"{' + AP' if lucky_config.align_mode == 2 else ''}")
            else:
                ...
                if new_result is not None:
                    ...
                    if self.use_advanced_stacking and self.advanced_stacker and drizzle_result is None:
                        ...
                        if master_stack is not None:
                            ...
                            # Mettre à jour la référence d'alignement Lucky avec le
                            # stack du buffer courant (new_result) plutôt que le master
                            # cumulatif : la référence reflète la position actuelle du
                            # ciel, ce qui compense mieux la dérive d'une table équatoriale
                            # à suivi médiocre (le master s'éloigne progressivement de la
                            # position réelle au fil des buffers).
                            self.lucky_stacker.update_alignment_reference(new_result)
